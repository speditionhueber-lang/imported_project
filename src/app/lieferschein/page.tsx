...
export default function LieferscheinPage() {
  ...
  const [gegenstaende, setGegenstaende] = useState<{name: string, count: string, montage: boolean}[]>([]);
  ...
  const finalJob = useMemo(() => {
      if (jobForLieferschein) return jobForLieferschein;
      
      const customerToUse = localCustomer || globalCustomer;
      if (customerToUse) {
        const customerState = customerStates[customerToUse.id];
        const defaultDate = customerToUse.umzugsdetails?.gewuenschterUmzugstermin
          ? new Date(customerToUse.umzugsdetails.gewuenschterUmzugstermin).toISOString()
          : new Date().toISOString();

          return {
              id: `job_mock_${customerToUse.id}`,
              customerName: customerToUse.name || '',
              scheduledAt: defaultDate,
              abholadresse: customerToUse.abholadresse,
              zieladresse: customerToUse.zieladresse,
              totalM3: customerState?.offerData?.totalM3 ?? 0, 
              calculatedHours: customerState?.offerData?.calculatedHours ?? 0, 
              status: 'draft', 
              notes: '', 
              createdAt: '',
              vehicles: [], 
  ...
  }, [jobForLieferschein, localCustomer, globalCustomer, customerStates]);


  useEffect(() => {
    const targetCustomer = localCustomer || globalCustomer;
    if (finalJob && targetCustomer) {
      setCustomerName(finalJob.customerName || targetCustomer.name || '');
      if (targetCustomer.address) {
        setCustomerStreet(targetCustomer.address.street || '');
        setCustomerCity(`${targetCustomer.address.zip || ''} ${targetCustomer.address.city || ''}`);
      }
      setScheduledAt(finalJob.scheduledAt);
      setAbholadresse(finalJob.abholadresse?.strasse || '');
      setZieladresse(finalJob.zieladresse?.strasse || '');
      setNote(finalJob.notes || '');
      setWorkers(finalJob.allocations?.[0]?.workers || []);
      setDocNumber(`LS-${Date.now().toString().slice(-6)}`);
      setTotalM3(finalJob.totalM3);

      if (finalJob.scheduledAt) {
          try {
              const jobStart = new Date(finalJob.scheduledAt);
              if (!isNaN(jobStart.getTime())) {
                  ...
                  if (startHour !== 0 || startMinute !== 0) {
                    ...
                    const end = addHours(jobStart, finalJob.calculatedHours || 9);
              ...
              console.error("Invalid date for job:", finalJob.id, finalJob.scheduledAt);
          }
      }

      const items = targetCustomer.gegenstaende
        ? Object.entries(targetCustomer.gegenstaende)
            .filter(([, value]) => {
            ...
            .map(([key, value]) => {
                ...
                return {
                    ...
                    count: String(value),
                    montage: false, // Initialize montage as false
                }
            })
        : [];
  ...
  const handleSaveAndContinue = () => {
    ...
    if (!customerForPdf) {
      toast({
        ...
        description: "Bitte wählen Sie einen Kunden aus, um den Lieferschein zu speichern."
      });
      return;
    }
    
    if (finalJob && logoBase64) {
  ...
  const handleAddItem = () => {
    setGegenstaende([...gegenstaende, { name: 'Neuer Gegenstand', count: '1', montage: false }]);
  }
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="w-[100px] text-center">Anzahl</TableHead>
                        ...
                        <TableBody>
                            {gegenstaende.length > 0 ? gegenstaende.map((item, index) => (
                                <TableRow key={index}>
                                    <TableCell><Input id={`item-name-${index}`} name={`itemName-${index}`} value={item.name} onChange={e => handleItemChange(index, 'name', e.target.value)}/></TableCell>
